...
//!
//! A save never loses anything: it refuses to overwrite a file whose SHA-256 changed since it
//! was loaded (another tool wrote it), copies the old contents to a timestamped backup (mode
//! 0600, the last N per file), and replaces the file atomically (a temp file in the same folder,
//! fsync, rename). Symlinks (a dotfiles repo's `~/.kube/config`) are followed, so the link
//! stays. New files and files with inline credentials are 0600; others keep their mode.
...
pub fn save(path: &Path, text: &str, options: &SaveOptions) -> Result<Saved, SaveError> {
    ...
    write_atomic(&written, text.as_bytes(), mode).map_err(io)?;
    Ok(Saved {
